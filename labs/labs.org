...
*** ~iterator~ and ~printInOrder~
    ...
    #+end_src 
    - ~Set~ *implements* ~Collection~, while ~Collection~ *implements* ~Iterable~ -> ~Set~ is *iterable*, ~Set.iterator~ to return ~Iterator~
    ...
    - Comparison among ~Arrays~, ~List~ and ~Set~ <<elink1>>
        ...
        - ~List~ <-> ~Arrays~
          - ~Arrays~ -> ~List~
            #+begin_src java :classname 
              ...
              List<String> list = Arrays.asList(s); <<ilink3>>
          ...
          - ~List~ -> ~Arrays~
            #+begin_src java :classname 
              String[] dest = list.toArray(new String[0]);//new String[0]是指定返回数组的类型 <<ilink4>>
      ...
      - ~Arrays.asList()~, ~Collection.toArray()~ and ~Iterable.iterator()~ 
        - ~Arrays.asList~
          ...
          - [[ilink3][ ~list~ ]]is an *instance* of ~java.util.Arrays$ArrayList~, not ~java.util.ArrayList~
        ...
        - ~Collection.toArray()~
          ...
          - ~<T> T[] toArray(T[] a)~: Returns an *array* containing all of the elements in this *collection*; the *runtime type* of the returned array is that of the specified array.
            ...
            #+end_src
            - *Runtime type* of [[ilink4][ ~dest~ ]] is ~String[]~, decided by ~new String[0]~ 
...
** Experiment 2 and 3 - BST Generation Using Insertion and Deletion :UNRESOLVED:EXTRA:
...
** Problem 2 Rabin-Karp Algorithm                          :UNRESOLVED:EXTRA:
** Question:                                                      :QUESTIONS:
...
*** ~isSeparable~ 
    ...
    - Runtime
      - $O(V+E)$
*** ~isSeparable3g~                                        :UNRESOLVED:EXTRA:
** Question:                                                      :QUESTIONS:
...
*** ~quicksort~
    - Recursive
      ...
      #+end_src
      - Base: ~if (items.size() <= 1) return items~ -> usual [[ilink2][base in recursive method for sorting]] 
...
*** ~TestSortAlgs.java~
    ...
    #+end_src 
* CLab 11 Bears and Beds                                         :UNRESOLVED:
** ~solvedBears~ 
   #+begin_src java :classname 
   ...
   #+end_src 
** Questions:                                                     :QUESTIONS:
   - [ ] 这道题应该意在通过两个列表之间的比较进行排序和配对，意图只在配对还是需要排序？怎么 *符合题意地* 实现？
